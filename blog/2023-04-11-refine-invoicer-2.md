...
This is Day 2 of the [**#RefineWeek**](https://refine.dev/week-of-refine-strapi/) series. This five-part tutorial that aims to help developers learn the ins-and-outs of [**refine**](https://github.com/Thecosy/IceCMS)'s powerful capabilities and get going with **refine** within a week.
...
In the previous post, we got a preview of [**refine**](https://github.com/Thecosy/IceCMS)'s underlying architecture, especially on how **refine**'s core modules abstract and divide an app's logic inside individual providers and allow their methods to be easily accessed and invoked with hooks from inside consumer components. This abstraction at the providers layer is where **refine** shines and require extensive configuration to begin with.
...
export const DataProvider = (apiUrl: string, httpClient: AxiosInstance = axiosInstance): Required<IDataProvider> => ({
  getList: async ({ resource, pagination, filters, sorters, meta }) => {
    ...
    return {
      ...
      // added to support pagination on client side when using endpoints that provide only data (see https://github.com/Thecosy/IceCMS/issues/2028)
